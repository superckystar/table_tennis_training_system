...
if(isset($_POST['add'])){
    ...
    $status = 'approved';
    $stmt->bind_param("ssssissssssi",$username_new,$password_new_hashed,$real_name_new,$gender_new,$age_new,$campus,$phone_new,$email_new,$achievements_new,$photo_path,$status,$level_new);
    $stmt->execute();
    $stmt->close();
    header("Location:a_manage_coaches.php?page=$page");
    exit();
}
...
if(isset($_POST['update'])){
    ...
    if(!empty($password_edit)){
        $stmt = $conn->prepare("UPDATE coaches SET username=?, password=?, real_name=?, gender=?, age=?, phone=?, email=?, achievements=?, photo=?, level=? WHERE id=? AND campus=?");
        $stmt->bind_param("ssssisssssisi",$username_edit,$password_hashed,$real_name,$gender,$age,$phone,$email,$achievements,$photo_path,$level,$id,$campus);
    }else{
        $stmt = $conn->prepare("UPDATE coaches SET username=?, real_name=?, gender=?, age=?, phone=?, email=?, achievements=?, photo=?, level=? WHERE id=? AND campus=?");
        $stmt->bind_param("sssisssssisi",$username_edit,$real_name,$gender,$age,$phone,$email,$achievements,$photo_path,$level,$id,$campus);
    }
    $stmt->execute();
    $stmt->close();
    header("Location:a_manage_coaches.php?page=$page");
...
// 获取教练列表
$result_count = $conn->query("SELECT COUNT(*) AS total FROM coaches WHERE campus='$campus'");
$total = $result_count->fetch_assoc()['total'];
$total_pages = ceil($total/$limit);
$offset = ($page-1)*$limit;
$coaches = $conn->query("SELECT * FROM coaches WHERE campus='$campus' LIMIT $limit OFFSET $offset");
?>
...
<h3 style="text-align:center;">
    <button onclick="window.location.href='a_success.php'">返回管理员主页</button>
</h3>
...
<form method="post" enctype="multipart/form-data">
    ...
    <div style="font-size:13px;color:#888;margin-top:-8px;margin-bottom:12px;">至少8位，包含字母、数字和特殊符号</div>
