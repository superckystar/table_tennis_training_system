...
if (isset($_POST['action']) && isset($_POST['appointment_id'])) {
    ...
    // 获取预约详细信息
    $stmt = $conn->prepare("SELECT student_id, coach_id, campus, paid FROM appointments WHERE id=?");
    $stmt->bind_param("i", $appointment_id);
    $stmt->execute();
    $stmt->bind_result($student_id, $coach_id, $campus, $paid);
    $stmt->fetch();
    $stmt->close();

    // 如果未付款，禁止操作
    if ($paid === 'no') {
        header("Location: manage_all_appointments.php?page=$page");
        exit();
    }
...
// 获取预约总数和分页
$result_count = $conn->query("SELECT COUNT(*) AS total FROM appointments");
$total = $result_count->fetch_assoc()['total'];
$total_pages = ceil($total / $limit);
$offset = ($page - 1) * $limit;

// 获取所有预约信息
$result = $conn->query("SELECT * FROM appointments ORDER BY date DESC, time_slot DESC LIMIT $limit OFFSET $offset");
?>
...
<head>
    ...
    <style>
        ...
        .container{max-width:1100px;margin:40px auto;padding:20px;background:#fff;border-radius:12px;box-shadow:0 4px 10px rgba(0,0,0,0.1);}
        ...
        .cancel-requested{background:#f39c12;color:#fff;}
        .cancel-requested:hover{background:#e67e22;}
                ...
                <tr>
                    ...
                    <td>
                        <form method="post" style="display:inline-block;">
                            ...
                            <?php else: ?>
                                <button type="submit" name="action" value="cancel" class="cancel disabled" disabled>不可操作</button>
                            <?php endif; ?>
                        </form>
                    </td>
                </tr>
